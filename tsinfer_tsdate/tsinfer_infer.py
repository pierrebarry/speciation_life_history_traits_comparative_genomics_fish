...
arg=sys.argv
#arg=['infer_tree.py','samples','1e-9','1e-2']
print(arg)

samples = tsinfer.load(arg[1])

ts = tsinfer.infer(samples,recombination_rate=float(arg[2]), mismatch_ratio=float(arg[3]))
print(
...
for sample_node_id in ts.samples():
  ...
  print(
    "Node",
    sample_node_id,
    "labels one chromosome LG7 sampled from individual",
    json.loads(ts.individual(individual_id).metadata),
    "in",
    json.loads(ts.population(population_id).metadata)["population"],
  )
